...
test('practice-first landing provides test and work tools without owner start mode', async ({ page }, info) => {
  ...
  await expect(page.getByRole('link', { name: 'GStep 홈', exact: true })).toBeVisible();
  await expect(page.getByRole('link', { name: /AI 대화 연습/ })).toBeVisible();
  await expect(page.getByRole('link', { name: /실제 GS25 교육 매뉴얼 기반/ })).toBeVisible();
  await expect(page.getByText(/GS25 공식 서비스가 아닙니다/)).toBeVisible();
