...
import { ArrowLeft, BookOpen, ClipboardCheck, Gamepad2, ChartColumn, Settings2, MessagesSquare } from 'lucide-react';
export function RoleShell({ role, children }: { role: 'crew' | 'manager'; children: React.ReactNode }) {
  const path = usePathname();
  const manager = role === 'manager';
  const links = manager ? [{label:'연습 기록·점수',href:'/manager/dashboard',icon:ChartColumn}, {label:'체크리스트 설정',href:'/manager/checklist',icon:ClipboardCheck}] : [{label:'시뮬레이터',href:'/crew/simulation',icon:Gamepad2}, {label:'AI 대화',href:'/crew/chat',icon:MessagesSquare}, {label:'매장 Q&A',href:'/crew/questions',icon:BookOpen}, {label:'체크리스트',href:'/crew/checklist',icon:ClipboardCheck}];
