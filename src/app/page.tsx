...
export default function Home() {
  return <div className="container service-home gstep-home">
    <div className="home-greeting"><span>스토어 매니저의 첫 성장, GStep</span><strong><Sparkles size={15} aria-hidden /> AI와 함께</strong></div>
    ...
    <section className="manual-promise" aria-labelledby="manual-heading"><div className="manual-promise-copy"><span className="ai-label"><BookOpenCheck size={15} aria-hidden /> 배움에는 근거가 있어야 하니까</span><h2 id="manual-heading">실제 업무의 순서를,<br />교육 매뉴얼에서 가져왔어요.</h2><p>GS25 공개 교육 자료 중 본문을 확인한 {passages.length}개 자료를 바탕으로, 근무 준비부터 상품·POS·고객 응대까지 연습합니다.</p><Link href="/sources">어떤 자료로 배우나요?<ArrowRight size={15} aria-hidden /></Link></div><ol className="ai-learning-flow"><li><span><BookOpenCheck size={19} aria-hidden /></span><div><strong>매뉴얼에서 상황을 만들고</strong><p>출처를 따라가며 업무의 이유를 이해해요.</p></div></li><li><span><BrainCircuit size={19} aria-hidden /></span><div><strong>AI가 내 응대를 읽고</strong><p>서술형·대화의 좋은 점과 보완점을 짚어요.</p></div></li><li><span><Check size={19} aria-hidden /></span><div><strong>다음 근무를 더 자신 있게</strong><p>정확도와 응답 시간을 보고 다시 연습해요.</p></div></li></ol></section>
