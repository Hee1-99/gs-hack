  ...
  return <>
    ...
    <div className={`checklist-items ${styles.items}`}>{state.checklistItems.map((item, index) => {
      ...
      return <article key={item.id} className={`checklist-row status-${status} ${styles.item} ${styles[status]}`}>
        ...
        <label className={`status-control ${styles.statusControl}`} htmlFor={`status-${item.id}`}><span className="sr-only">{item.title} 상태</span><select id={`status-${item.id}`} value={status} onChange={event => { const result = repo.setChecklistStatus(item.id, checklistStatusSchema.parse(event.target.value)); setMessage(result.persisted ? '업무 상태를 저장했어요.' : '브라우저에 저장하지 못했어요. 현재 화면에서만 임시로 적용돼요.'); }}><option value="pending">대기</option><option value="done">완료</option><option value="needs_manager">경영주 확인 필요</option></select></label>
      </article>;
    })}</div>
