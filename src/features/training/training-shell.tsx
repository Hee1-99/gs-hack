...
import { ArrowLeft, ArrowRight, Barcode, Check, CheckCircle2, ClipboardCheck, Clock3, CreditCard, FileText, GraduationCap, MessagesSquare, Monitor, PackageCheck, Play, RotateCcw, ScanLine, ShieldCheck, ShoppingBag, Sparkles, X } from 'lucide-react';
...
function usesPos(step: TrainingStep) { return ['scan', 'promotion', 'discount', 'payment', 'receipt', 'hold'].includes(step.screen); }
...
function CounterScene({ step, answered }: { step: TrainingStep; answered: boolean }) {
  return <div className={`counter-scene ${answered ? 'scene-answered' : ''}`} aria-label="가상 매장 카운터 장면" role="img">
    ...
    <div className="scene-bubble" key={step.id}><span>{step.speakerLabel ?? (step.chapter === '근무 준비' || step.screen === 'waste' || step.screen === 'report' ? '함께 일하는 스토어 매니저' : '고객')}</span><p>{step.customer}</p></div>
...
function PosDisplay({ step, answered, selected, onAction, disabled }: { step: TrainingStep; answered: boolean; selected: string | null; onAction: (id: string) => void; disabled: boolean }) {
  const isProducts = ['scan', 'promotion', 'discount', 'payment', 'receipt', 'hold'].includes(step.id);
  ...
  return <div className={`training-pos ${answered ? 'pos-updated' : ''}`}>
    <div className="pos-top"><span><Monitor size={15}/>{usesPos(step) ? 'GStep POS' : 'GStep 업무 확인'}</span><span className="pos-online"><i/>연습 전용</span></div>
    <div className="pos-app">
      <div className="pos-toolbar"><span className="pos-tab">{isProducts ? '상품 판매' : step.chapter}</span><span>가상 매장 01</span></div>
      <div className="pos-workspace">
        ...
        </> : <div className="pos-task-display"><div className="pos-task-icon">{step.chapter === '근무 준비' ? <ClipboardCheck size={34}/> : step.screen === 'waste' ? <PackageCheck size={34}/> : <FileText size={34}/>}</div><strong>{step.title}</strong><p>{step.chapter} · 상황을 확인하고 판단해요</p><span>아래에서 다음 행동을 선택하세요</span></div>}
      </div>
      <div className="pos-actions" role="group" aria-label="행동 선택">
...
export function TrainingShell({initialMode='practice'}:{initialMode?:TrainingMode}) {
  ...
  return <div className="training-shell">
    ...
    {showResult&&session?<TrainingResult attempt={session} onRetry={()=>begin('practice',new Set(steps.map(item=>item.chapter)).size===1?steps[0].chapter:'all')}/>:session&&step?<>
      ...
      <div className="training-arena"><section className="training-customer"><CounterScene step={step} answered={Boolean(feedback)}/><div className="training-prompt"><span>이번 단계의 미션</span><h2>{step.question}</h2><p>{step.mission ?? (step.kind==='short-answer'?'상대에게 전달할 내용을 직접 적어 주세요.':'상황에 맞는 다음 행동을 선택해 주세요.')}</p></div><div className="training-source-note"><FileText size={14}/><span>{session.mode==='test'?'테스트 중에는 정답과 해설이 표시되지 않아요.':`교육 근거 · ${step.source.title}`}</span></div></section><section className="training-terminal" aria-label={step.kind === 'short-answer' ? '서술형 답안 작성' : usesPos(step) ? '가상 POS' : '업무 행동 선택 화면'}>
      {step.kind==='short-answer'?<div className="training-written"><span className="pill">직접 쓰는 상황 답안</span><label htmlFor="training-answer">{step.responseLabel ?? '상대에게 전달할 내용'}</label><textarea id="training-answer" maxLength={1500} rows={6} value={answerText} disabled={pending||Boolean(feedback)} onChange={event=>{setAnswerText(event.target.value);if(!error)submittedElapsed.current=null;}} placeholder={step.answerPlaceholder ?? '확인할 내용과 다음 행동을 자연스럽게 설명해 보세요.'}/><p>말투만 보지 않고, 상황 확인과 필요한 행동이 담겼는지 평가해요.</p>{session.mode==='practice'&&<details><summary>채점 기준 보기</summary><ul>{step.rubric?.map(item=><li key={item.id}>{item.label}</li>)}</ul></details>}</div>:<PosDisplay step={step} answered={Boolean(feedback?.correct)} selected={selected??feedback?.choiceId??null} onAction={id=>{setSelected(id);selectedAt.current=Date.now();submittedElapsed.current=null;setClock(Date.now());}} disabled={pending||Boolean(feedback)}/>}
      {!feedback&&<div className="training-submit">{pending?<div className="training-grading" role="status"><div className="grading-orbit"><Sparkles size={24}/></div><strong>{step.kind==='short-answer'?'AI 채점을 요청하고 있어요':'답안과 시간을 확인 중이에요'}</strong><span>매뉴얼의 기준을 확인하고 있어요.</span></div>:<p aria-live="polite">{step.kind==='short-answer'?`${answerText.length} / 1500자`:selected?<><CheckCircle2 size={16}/>{step.choices.find(choice=>choice.id===selected)?.label}</>:'상황에 맞는 다음 행동을 골라 주세요.'}</p>}{error&&<p role="alert">{error}</p>}<button className="button" disabled={pending||(step.kind==='short-answer'?!answerText.trim():!selected)} onClick={()=>void submit()}>{pending?'채점 중…':session.mode==='test'?index===steps.length-1?'제출하고 점수 보기':'답 제출하고 다음 단계':step.kind==='short-answer'?'답안 제출하기':'이 행동으로 진행'}<ArrowRight size={17}/></button></div>}</section></div>
      {feedback&&<section className={`training-feedback ${feedback.correct?'correct':'incorrect'}`} aria-live="polite"><div className="feedback-symbol">{feedback.correct?<Check size={23}/>:<RotateCcw size={23}/>}</div><div><h2 ref={feedbackHeading} tabIndex={-1}>{feedback.correct?'좋아요, 필요한 행동을 짚었어요!':'이 단계는 다시 기억해 두세요.'}</h2>{step.kind==='short-answer'?<><p><strong>{feedback.gradingMode==='gemini'?'AI 의미 채점':'기본 기준 채점 · 데모'} · {feedback.accuracyPoints}점</strong></p><p>{feedback.feedback}</p></>:!feedback.correct&&<p><strong>{step.choices.find(choice=>choice.id===step.correctChoiceId)?.label}</strong></p>}<p>{step.explanation}</p>{feedback.elapsedMs!==undefined&&<p>{duration(feedback.elapsedMs)} · 시간 보너스 {feedback.timeBonus}점</p>}<a href={step.source.url} target="_blank" rel="noreferrer">{step.source.title} · 교육 자료 보기 ↗</a></div><button className="button" onClick={advance}>{session.status==='completed'?'최종 점수 보기':'다음 단계'}<ArrowRight size={16}/></button></section>}
      ...
      <div className="training-mode-switch" aria-label="연습 방식"><button className={mode==='practice'?'active':''} aria-pressed={mode==='practice'} onClick={()=>setMode('practice')}><GraduationCap size={19}/>차근차근 연습</button><button className={mode==='test'?'active':''} aria-pressed={mode==='test'} onClick={()=>setMode('test')}><ShieldCheck size={19}/>채용 테스트</button><Link href="/crew/chat"><MessagesSquare size={19}/>AI 응대 연습</Link></div>
