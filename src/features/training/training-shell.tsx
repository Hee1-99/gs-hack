...
export function TrainingShell({initialMode='practice'}:{initialMode?:TrainingMode}) {
  ...
  return <div className="training-shell">
    ...
    {showResult&&session?<TrainingResult attempt={session} onRetry={()=>begin('practice',new Set(steps.map(item=>item.chapter)).size===1?steps[0].chapter:'all')}/>:session&&step?<>
      ...
      {feedback&&<section className={`training-feedback ${feedback.correct?'correct':'incorrect'}`} aria-live="polite"><div className="feedback-symbol">{feedback.correct?<Check size={23}/>:<RotateCcw size={23}/>}</div><div><h2 ref={feedbackHeading} tabIndex={-1}>{feedback.correct?'좋아요, 필요한 행동을 짚었어요!':'이 단계는 다시 기억해 두세요.'}</h2>{step.kind==='short-answer'?<><p><strong>{feedback.gradingMode==='gemini'?'Gemini 의미 채점':'기본 기준 채점 · 데모'} · {feedback.accuracyPoints}점</strong></p><p>{feedback.feedback}</p></>:!feedback.correct&&<p><strong>{step.choices.find(choice=>choice.id===step.correctChoiceId)?.label}</strong></p>}<p>{step.explanation}</p>{feedback.elapsedMs!==undefined&&<p>{duration(feedback.elapsedMs)} · 시간 보너스 {feedback.timeBonus}점</p>}<a href={step.source.url} target="_blank" rel="noreferrer">{step.source.title} · 교육 자료 보기 ↗</a></div><button className="button" onClick={advance}>{session.status==='completed'?'최종 점수 보기':'다음 단계'}<ArrowRight size={16}/></button></section>}
