...
function RuleForm({ rule, repo, onCreated }: { rule?: StoreRule; repo: StoreRepository; onCreated?: () => void }) {
  ...
  return <form className="rule-form panel" aria-label={rule?.title ?? '새 규칙 작성'} onSubmit={async event => {
    ...
    setSaving(true);
    try {
      const result = await (rule ? repo.updateRule(rule.id, parsed.data) : repo.createRule(parsed.data));
      setMessage(result.persisted ? '저장했어요. 새로운 질문과 연습부터 적용돼요.' : '브라우저에 저장하지 못했어요. 현재 화면에서만 임시로 적용돼요.');
      if (!rule && result.persisted) onCreated?.();
    } catch { setMessage('저장하지 못했어요. 입력 내용을 확인한 뒤 다시 시도해 주세요.'); } finally { setSaving(false); }
  }}>
    ...
    <div className="form-actions"><button className="button" type="submit" disabled={saving}>{rule ? '규칙 저장' : '규칙 등록'}</button><span role="status" className="save-message">{message}</span></div>
  </form>;
}
