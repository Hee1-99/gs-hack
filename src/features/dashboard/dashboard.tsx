...
export function Dashboard() {
  ...
  return <>
    ...
    {auth.user && <div className={styles.cloudStatus}><span>{auth.membership?.store_name} · 연결된 매장의 기록</span><button className="button secondary" disabled={cloud.loading} onClick={() => void cloud.refresh()}>{cloud.loading ? '불러오는 중…' : '기록 새로고침'}</button></div>}
      ...
      {!records.length ? <div className={styles.empty}><ClipboardCheck size={36} aria-hidden /><h3>{filter === 'all' ? '아직 연습 기록이 없어요' : `${filter === 'test' ? '구인 테스트' : '연습'} 기록이 없어요`}</h3><p>스토어 매니저가 시뮬레이터를 진행하면 단계별 결과가 여기에 저장돼요.</p><Link href="/crew/simulation" className="button">시뮬레이터 열기<ArrowRight size={16} aria-hidden /></Link></div> : <div className={styles.records}>{records.map(attempt => <details key={attempt.id} className={styles.record}>
        ...
        <div className={styles.resultBody}><p className={styles.resultLabel}>{attempt.status === 'completed' ? `${attempt.answers.filter(answer => answer.correct).length}개 기준 충족 · ${getAttemptSteps(attempt).length}개 단계 완료` : `${attempt.answers.length}개 단계 응답 · 완료 후 점수가 표시돼요`}</p>
          ...
          {!attempt.answers.length ? <p className="small-note">아직 제출한 답변이 없어요.</p> : <ol className={styles.steps}>{attempt.answers.map(answer => {
            ...
            return <li key={answer.stepId}><span className={answer.correct ? styles.correct : styles.incorrect}>{answer.correct ? <Check size={15} aria-hidden /> : <X size={15} aria-hidden />}<span className="sr-only">{answer.correct ? '정답' : '오답'}</span></span><div><strong>{step?.title ?? answer.stepId}</strong><p>{answer.answerText ?? step?.choices.find(choice => choice.id === answer.choiceId)?.label ?? '선택한 답변'}</p>{answer.feedback && <p className={styles.aiFeedback}>{answer.gradingMode === 'gemini' ? 'Gemini 피드백' : '데모 피드백'} · {answer.feedback}</p>}{answer.elapsedMs !== undefined && <span className={styles.answerTime}>{Math.round(answer.elapsedMs / 1000)}초 · 정확도 {answer.accuracyPoints}점{answer.gradingMode === 'gemini' ? ' · AI 채점' : ''}</span>}{step && <a href={step.source.url} target="_blank" rel="noreferrer">교육 근거 · {step.source.title}</a>}</div></li>;
          })}</ol>}
        </div>
      </details>)}</div>}
