...
export function LoginScreen() {
 ...
 async function credentials(event:React.FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   if(result.error){setMessage(result.error.code==='email_not_confirmed'?'계정 인증 설정을 확인해 주세요. 운영자가 이메일 확인을 꺼야 아이디로 바로 로그인할 수 있어요.':result.error.code==='user_already_exists'?'이미 사용 중인 아이디예요. 다른 아이디를 입력해 주세요.':mode==='login'?'아이디와 비밀번호를 확인해 주세요.':'가입하지 못했어요. 이미 사용 중인 아이디인지 확인하고 다시 시도해 주세요.');return;}
   if(mode==='signup'&&!result.data.session)setMessage('바로 로그인하지 못했어요. 운영자에게 계정 인증 설정을 확인해 주세요.');
   else await auth.refreshMembership();
 ...
 return <div className={`container ${styles.page}`}><div className="page-heading"><p className="eyebrow">나의 연습을 이어서</p><h1>{auth.user?'내 계정과 매장':'로그인'}</h1><p>매장을 연결하면 연습 기록과 체크리스트를 함께 확인해요.</p></div>
