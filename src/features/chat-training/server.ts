...
function demoFeedback(request: ChatRequest, sourceIds: string[]): ChatFeedback {
  ...
  const criteria = criterionIds.map((id, index) => { const present = patterns[index].test(said); return { id, score: present ? 20 : 5, maxScore: 25 as const, comment: present ? `${criterionLabels[id]} 관련 표현이 대화에 있었어요. 실제 맥락에 맞는지도 다시 읽어 보세요.` : `${criterionLabels[id]}를 보여 주는 구체적인 표현을 더해 보세요.` }; });
  return { score: criteria.reduce((sum, item) => sum + item.score, 0), summary: '데모의 간단한 표현 점검 결과예요. 실제 AI의 문맥 평가와는 다르며, 말의 의미와 상황을 함께 복습해 주세요.', criteria, strengths: [criteria.filter(item => item.score >= 20).map(item => criterionLabels[item.id]).join(', ') || '고객에게 직접 답변하며 연습을 시작했어요.'], improvements: ['확인하지 않은 처리나 보상을 약속하지 말고, 확인할 내용과 다음 행동을 구체적으로 설명해 보세요.'], sourceIds };
}
...
function isNaturalCustomerClosure(message: string) {
  const asksForMore = /[?？]|알려|무엇|어떻게|확인 부탁|보여|드리면|주실 수/.test(message);
  return !asksForMore && /감사|알겠|그렇게 할게|괜찮아요|됐어요/.test(message);
}
export async function handleChatTraining(request: ChatRequest, options: AiOptions): Promise<ChatResponse> {
  ...
  if (request.action === 'reply') {
    ...
    const message = unsafeNumber ? fallback.message : result.data.message;
    return { message, shouldFinish: isNaturalCustomerClosure(message), mode: result.mode === 'gemini' && !unsafeNumber ? 'live' : 'demo', sources, facts };
  }
  ...
  return { feedback, shouldFinish: true, mode: result.mode === 'gemini' ? 'live' : 'demo', sources, facts };
}
