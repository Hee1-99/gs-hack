...
export const chatResponseSchema = z.object({ message: z.string().min(1).max(1200).optional(), feedback: feedbackSchema.optional(), mode: z.enum(['live', 'demo']), sources: z.array(manualSourceSchema).max(3), facts: z.array(z.string().max(500)).max(4) });
