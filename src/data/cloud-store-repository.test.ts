...
function backend(){
 ...
 const from=vi.fn((table:string)=>{const response={data:table==='gstep_store_content'?{rules:seed.rules,checklist_items:seed.checklistItems,version:1}:null,error:null}; const query={select:()=>query,eq:()=>query,single:async()=>response,maybeSingle:async()=>response};return query;});
