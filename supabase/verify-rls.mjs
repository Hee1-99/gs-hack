...
try {
 ...
 await check('owner shared content write succeeds only within own store and version',async()=>{await as(ownerA);assert.equal(await scalar("select public.gstep_save_content($1,1,'[]','[]')",[storeA]),2);await fails("select public.gstep_save_content($1,1,'[]','[]')",[storeA],'P0001');await fails("select public.gstep_save_content($1,1,'[]','[]')",[storeB]);});
